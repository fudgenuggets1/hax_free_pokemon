...
def pokemon_type_advantage(pokemon1, pokemon2):
	...
	type_advantage /= 2
	if offense_advantage == 0 or offense_advantage == 4:
		type_advantage = offense_advantage
	if defense_advantage == 4 or defense_advantage == 0:
		type_advantage = defense_advantage
	return (offense_advantage, defense_advantage)
...
def best_switch(pokemon, opponent_party, dead=False):
	...
	if choice1.current_health > 0:
		...
		if choice1.current_health - damage > 0:	
	...
	if choice2.current_health > 0:
		...
		if choice2.current_health - damage > 0:		
	...
	if choice3.current_health > 0:
		...
		if choice3.current_health - damage > 0:
...
def who_wins(pokemon, opponent):
	test_pokemon_health = pokemon.current_health
	test_opponent_health = opponent.current_health
	move = opponent_move(opponent, pokemon)
	type_advantage = move_type_advantage(opponent, move)
	pokemon_damage = damage_calc(pokemon, opponent, move, type_advantage)
	move = opponent_move(pokemon, opponent)
	type_advantage = move_type_advantage(pokemon, move)
	opponent_damage = damage_calc(opponent, pokemon, move, type_advantage)
	if pokemon.base_speed > opponent.base_speed:
